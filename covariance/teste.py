...
# calculo correlacao ==================================================
def sum_dev_squares(lista):
    mean = media(lista)
    return sum([((x - mean) ** 2) for x in lista])

def autocorrelacao(valores):
    # valores_delta = delta(valores)
    return (autocovariancia(valores) * (len(valores) - 1)) / sum_dev_squares(valores)
    # return covariancia_temporal(valores) / (np.std(valores_delta) ** 2)
